"""강수강도 버킷 격자(앱의 강수 타이밍 배너 입력) — 예측(QPF)과 실황(HSP) 둘 다.

예측은 이미 색이 입혀진 팔레트 이미지로 오므로 RGB 우세색으로 버킷을 결정하고
(정확 색매칭 테이블 없이 결정론적), 실황은 원값(mm/h×100)이 있으니 값으로 끊는다
(아래 hsp_level_of). 두 경로 모두 0=무강수, 1=약, 2=보통, 3=강, 4=매우강 —
같은 레벨 축이라야 앱이 '지금(실황)'과 '이따(예측)'를 한 자로 잴 수 있다.
...
# ── 실황(HSP) → 레벨 ────────────────────────────────────────────────────────
# HSP는 int16, 값 = mm/h × 100 (NULL_V=-30000, NORAIN_V=-25000; grid.py 참조).
# 예측과 달리 팔레트 RGB를 되읽지 않고 원값에서 바로 끊는다 — 원값이 있는데 색을
# 거쳐 되짚을 이유가 없다.
#
# 임계값 유도(재검증용): colormap_hsp.txt 의 정지점 색을 위 level_of 의미
# (파랑=1, 초록=2, 노랑·주황=3, 빨강·보라=4)로 밀어넣으면 색이 바뀌는 자리가 그대로
# 레벨 경계가 된다 — 사용자가 관측 프레임에서 '보는 색'과 앱이 '받는 레벨'이
# 어긋나지 않게 하는 게 목적이다.
#
#   v < 10   : 알파 0 (5→a=0, 10→a=110)                     → 0 무강수(<0.1mm/h)
#   10~200   : 110,190,255 / 70,160,255 / 30,120,255 = 파랑 → 1 약(0.1~2.0)
#   200~600  : 0,200,170 / 0,190,70 / 80,210,0     = 초록   → 2 보통(2.0~6.0)
#   600~1200 : 255,230,0 / 255,170,0            = 노랑·주황 → 3 강(6.0~12.0)
#   v >= 1200: 255,100,0 … 215,0,40 / 195,0,165 = 빨강·보라 → 4 매우강(12.0~)
#
# (팔레트 맨 위 15000=125,70,255 만은 level_of 의 RGB 휴리스틱에 넣으면 r<128 이라
#  1로 새지만, 그건 이산 팔레트용 휴리스틱의 한계일 뿐 150mm/h 는 당연히 4다.
#  원값 임계로 끊는 이 경로엔 그 함정이 없다.)
# NULL_V·NORAIN_V 는 10 미만이라 별도 분기 없이 0으로 떨어진다.
HSP_BREAKS = (10, 200, 600, 1200)


def hsp_level_of(v: int) -> int:
    """실황 원값(mm/h×100) → 레벨 0~4."""
    return int(np.searchsorted(HSP_BREAKS, int(v), side="right"))


def hsp_levels_grid(arr: np.ndarray) -> np.ndarray:
    """(H,W) int16 실황 배열 → (H,W) int16 레벨. 벡터화."""
    return np.searchsorted(HSP_BREAKS, arr, side="right").astype(np.int16)
